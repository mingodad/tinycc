...
static void asm_expr_unary(TCCState *tcc_state, ExprValue *pe)
{
    ...
    case TOK_PPNUM:
        p = tcc_state->tccpp_tokc.cstr->data;
    ...
    default:
        if (tcc_state->tccpp_tok >= TOK_IDENT) {
            ...
            if (!sym) {
                sym = label_push(tcc_state, &tcc_state->asm_labels, tcc_state->tccpp_tok, 0);
...
{
    asm_new_label1(tcc_state, label, is_local, tcc_state->tccgen_cur_text_section->sh_num, tcc_state->tccgen_ind);
}
...
static void asm_free_labels(TCCState *tcc_state)
{
    ...
    for(s = tcc_state->asm_labels; s != NULL; s = s1) {
        s1 = s->prev;
        ...
        /* remove label */
        tcc_state->tccpp_table_ident[s->v - TOK_IDENT]->sym_label = NULL;
...
static void use_section1(TCCState *tcc_state, Section *sec)
{
    ...
    tcc_state->tccgen_cur_text_section = sec;
...
static void asm_parse_directive(TCCState *tcc_state)
{
    ...
    /* assembler directive */
    next(tcc_state);
    sec = tcc_state->tccgen_cur_text_section;
    switch(tcc_state->tccpp_tok) {
    case TOK_ASM_align:
    case TOK_ASM_skip:
    case TOK_ASM_space:
        tok1 = tcc_state->tccpp_tok;
        next(tcc_state);
        n = asm_int_expr(tcc_state);
        if (tok1 == TOK_ASM_align) {
            if (n < 0 || (n & (n-1)) != 0)
                tcc_error(tcc_state, "alignment must be a positive power of two");
            offset = (tcc_state->tccgen_ind + n - 1) & -n;
            size = offset - tcc_state->tccgen_ind;
    ...
    zero_pad:
        if (sec->sh_type != SHT_NOBITS) {
            sec->data_offset = tcc_state->tccgen_ind;
            ptr = section_ptr_add(tcc_state, sec, size);
            memset(ptr, v, size);
        }
        tcc_state->tccgen_ind += size;
        break;
    case TOK_ASM_quad:
        next(tcc_state);
        for(;;) {
    ...
    case TOK_ASM_org:
        {
            ...
            n = asm_int_expr(tcc_state);
            if (n < tcc_state->tccgen_ind)
                tcc_error(tcc_state, "attempt to .org backwards");
            v = 0;
            size = n - tcc_state->tccgen_ind;
            goto zero_pad;
        }
        break;
    case TOK_ASM_globl:
    ...
    case TOK_ASM_asciz:
        {
            ...
            for(;;) {
                ...
                p = tcc_state->tccpp_tokc.cstr->data;
                size = tcc_state->tccpp_tokc.cstr->size;
                ...
                if (tcc_state->tccpp_tok == ',') {
                    next(tcc_state);
                } else if (tcc_state->tccpp_tok != TOK_STR) {
                    break;
                }
            }
	}
	...
	{ 
            ...
            n = 0;
            next(tcc_state);
            if (tcc_state->tccpp_tok != ';' && tcc_state->tccpp_tok != TOK_LINEFEED) {
		n = asm_int_expr(tcc_state);
		next(tcc_state);
            }
    ...
    case TOK_ASM_tccpp_file:
        {
            ...
            if (tcc_state->tccpp_tok == TOK_STR)
                pstrcat(filename, sizeof(filename), tcc_state->tccpp_tokc.cstr->data);
            else
                pstrcat(filename, sizeof(filename), get_tok_str(tcc_state, tcc_state->tccpp_tok, NULL));
    ...
    case TOK_ASM_ident:
        {
            ...
            if (tcc_state->tccpp_tok == TOK_STR)
                pstrcat(ident, sizeof(ident), tcc_state->tccpp_tokc.cstr->data);
            else
                pstrcat(ident, sizeof(ident), get_tok_str(tcc_state, tcc_state->tccpp_tok, NULL));
    ...
    case TOK_ASM_size:
        { 
            ...
            if (tcc_state->warn_unsupported)
                tcc_warning(tcc_state, "ignoring .size %s,*", get_tok_str(tcc_state, tcc_state->tccpp_tok, NULL));

            while (tcc_state->tccpp_tok != '\n' && tcc_state->tccpp_tok != CH_EOF) {
                next(tcc_state);
            }
        }
        break;
    case TOK_ASM_type:
        { 
            ...
            skip(tcc_state, ',');
            if (tcc_state->tccpp_tok == TOK_STR) {
                newtype = tcc_state->tccpp_tokc.cstr->data;
            } else {
                if (tcc_state->tccpp_tok == '@' || tcc_state->tccpp_tok == '%')
                    skip(tcc_state, tcc_state->tccpp_tok);
                newtype = get_tok_str(tcc_state, tcc_state->tccpp_tok, NULL);
            }
    ...
    case TOK_SECTION1:
        {
            ...
            next(tcc_state);
            sname[0] = '\0';
            while (tcc_state->tccpp_tok != ';' && tcc_state->tccpp_tok != TOK_LINEFEED && tcc_state->tccpp_tok != ',') {
                if (tcc_state->tccpp_tok == TOK_STR)
                    pstrcat(sname, sizeof(sname), tcc_state->tccpp_tokc.cstr->data);
                else
                    pstrcat(sname, sizeof(sname), get_tok_str(tcc_state, tcc_state->tccpp_tok, NULL));
                next(tcc_state);
            }
            if (tcc_state->tccpp_tok == ',') {
            ...
            tcc_state->tccgen_last_text_section = tcc_state->tccgen_cur_text_section;
            use_section(tcc_state, sname);
        }
        break;
    case TOK_ASM_previous:
        { 
            Section *sec;
            next(tcc_state);
            if (!tcc_state->tccgen_last_text_section)
                tcc_error(tcc_state, "no previous section referenced");
            sec = tcc_state->tccgen_cur_text_section;
            use_section1(tcc_state, tcc_state->tccgen_last_text_section);
            tcc_state->tccgen_last_text_section = sec;
        }
        break;
...
static int tcc_assemble_internal(TCCState *tcc_state, int do_preprocess)
{
    ...
    tcc_state->tccpp_parse_flags = PARSE_FLAG_ASM_COMMENTS;
    if (do_preprocess)
    ...
    for(;;) {
        if (tcc_state->tccpp_tok == TOK_EOF)
            break;
        tcc_state->tccpp_parse_flags |= PARSE_FLAG_LINEFEED; /* XXX: suppress that hack */
    redo:
        if (tcc_state->tccpp_tok == '#') {
        ...
        } else if (tcc_state->tccpp_tok == TOK_PPNUM) {
            const char *p;
            int n;
            p = tcc_state->tccpp_tokc.cstr->data;
...
static void tcc_assemble_inline(TCCState *tcc_state, char *str, int len)
{
    ...
    saved_macro_ptr = tcc_state->tccpp_macro_ptr;
    ...
    tcc_state->tccpp_macro_ptr = saved_macro_ptr;
}
    ...
    if (tcc_state->tccpp_tok != ':') {
        ...
        for(;;) {
            ...
            if (tcc_state->tccpp_tok == '[') {
                ...
                op->id = tcc_state->tccpp_tok;
                next(tcc_state);
                skip(tcc_state, ']');
            }
            if (tcc_state->tccpp_tok != TOK_STR)
                expect(tcc_state, "string constant");
            op->constraint = tcc_malloc(tcc_state, tcc_state->tccpp_tokc.cstr->size);
            strcpy(op->constraint, tcc_state->tccpp_tokc.cstr->data);
            next(tcc_state);
            skip(tcc_state, '(');
            gexpr(tcc_state);
            if (is_output) {
                ...
                if ((tcc_state->tccgen_vtop->r & VT_LVAL) &&
                    ((tcc_state->tccgen_vtop->r & VT_VALMASK) == VT_LLOCAL ||
                     (tcc_state->tccgen_vtop->r & VT_VALMASK) < VT_CONST) &&
                    !strchr(op->constraint, 'm')) {
                    gv(tcc_state, RC_INT);
                }
            }
            op->vt = tcc_state->tccgen_vtop;
            skip(tcc_state, ')');
            if (tcc_state->tccpp_tok == ',') {
                next(tcc_state);
            } else {
...
ST_FUNC void asm_instr(TCCState* tcc_state)
{
    ...
    /* since we always generate the asm() instruction, we can ignore
       volatile */
    if (tcc_state->tccpp_tok == TOK_VOLATILE1 || tcc_state->tccpp_tok == TOK_VOLATILE2 || tcc_state->tccpp_tok == TOK_VOLATILE3) {
        next(tcc_state);
    }
            ...
            if (tcc_state->tccpp_tok != ')') {
                ...
                if (tcc_state->tccpp_tok == ':') {
                    ...
                    for(;;) {
                        if (tcc_state->tccpp_tok != TOK_STR)
                            expect(tcc_state, "string constant");
                        asm_clobber(tcc_state, clobber_regs, tcc_state->tccpp_tokc.cstr->data);
                        next(tcc_state);
                        if (tcc_state->tccpp_tok == ',') {
                            next(tcc_state);
                        } else {
