...
{
    if (r & VT_SYM)
        greloc(tcc_state, tcc_state->tccgen_cur_text_section, sym, tcc_state->tccgen_ind, R_386_32);
...
{
    if (r & VT_SYM)
        greloc(tcc_state, tcc_state->tccgen_cur_text_section, sym, tcc_state->tccgen_ind, R_386_PC32);
...
static void gen_static_call(TCCState* tcc_state, int v)
{
    ...
    sym = external_global_sym(tcc_state, v, &tcc_state->tccgen_func_old_type, 0);
    oad(tcc_state, 0xe8, -4);
    greloc(tcc_state, tcc_state->tccgen_cur_text_section, sym, tcc_state->tccgen_ind-4, R_386_PC32);
}
...
ST_FUNC void gfunc_call(TCCState* tcc_state, int nb_args)
{
    ...
    for(i = 0;i < nb_args; i++) {
        if ((tcc_state->tccgen_vtop->type.t & VT_BTYPE) == VT_STRUCT) {
            ...
            vstore(tcc_state);
            args_size += size;
        } else if (is_float(tcc_state->tccgen_vtop->type.t)) {
            ...
            if ((tcc_state->tccgen_vtop->type.t & VT_BTYPE) == VT_LLONG) {
                size = 8;
                o(tcc_state, 0x50 + tcc_state->tccgen_vtop->r2); /* push r */
            } else {
                size = 4;
            }
    ...
    func_sym = tcc_state->tccgen_vtop->type.ref;
    ...
    else if ((tcc_state->tccgen_vtop->type.ref->type.t & VT_BTYPE) == VT_STRUCT)
        args_size -= 4;
#endif
...
ST_FUNC void gfunc_prolog(TCCState* tcc_state, CType *func_type)
{
    ...
    if (((tcc_state->tccgen_func_vt.t & VT_BTYPE) == VT_STRUCT) && (size > 8)) {
#else
    ...
    while ((sym = sym->next) != NULL) {
        ...
        if (param_index < fastcall_nb_regs) {
            ...
            tcc_state->tccgen_loc -= 4;
            o(tcc_state, 0x89);     /* movl */
            gen_modrm(tcc_state, fastcall_regs_ptr[param_index], VT_LOCAL, NULL, tcc_state->tccgen_loc);
...
ST_FUNC void gen_opi(TCCState* tcc_state, int op)
{
    ...
    case TOK_UMULL:
        ...
        if (op == TOK_UMULL) {
            ...
            o(tcc_state, 0xe0 + fr);
            tcc_state->tccgen_vtop->r2 = TREG_EDX;
            r = TREG_EAX;
        } else {
    ...
    if (op >= TOK_ULT && op <= TOK_GT) {
        ...
        if ((ft & VT_BTYPE) == VT_LDOUBLE) {
            ...
            if ((ft & VT_BTYPE) == VT_DOUBLE)
                ...
                o(tcc_state, 0xd8);
            gen_modrm(tcc_state, a, r, tcc_state->tccgen_vtop->sym, fc);
        }
        tcc_state->tccgen_vtop--;
    }
}
...
ST_FUNC void gen_cvt_itof(TCCState* tcc_state, int t)
{
    ...
    if ((tcc_state->tccgen_vtop->type.t & VT_BTYPE) == VT_LLONG) {
        ...
        o(tcc_state, 0x50 + tcc_state->tccgen_vtop->r2); /* push r2 */
...
{
    int bt = tcc_state->tccgen_vtop->type.t & VT_BTYPE;
    if (bt == VT_FLOAT)
        vpush_global_sym(tcc_state, &tcc_state->tccgen_func_old_type, TOK___fixsfdi);
    else if (bt == VT_LDOUBLE)
        vpush_global_sym(tcc_state, &tcc_state->tccgen_func_old_type, TOK___fixxfdi);
    else
        vpush_global_sym(tcc_state, &tcc_state->tccgen_func_old_type, TOK___fixdfdi);
    vswap(tcc_state);
...
ST_FUNC void gen_bounded_ptr_add(TCCState* tcc_state)
{
    ...
    tcc_state->tccgen_vtop->c.ul = (tcc_state->tccgen_cur_text_section->reloc->data_offset - sizeof(Elf32_Rel)); 
}
...
ST_FUNC void gen_bounded_ptr_deref(TCCState* tcc_state)
{
    ...
    if (!is_float(tcc_state->tccgen_vtop->type.t)) {
        if (tcc_state->tccgen_vtop->r & VT_LVAL_BYTE)
            size = 1;
    ...
    rel = (Elf32_Rel *)(tcc_state->tccgen_cur_text_section->reloc->data + tcc_state->tccgen_vtop->c.ul);
    sym = external_global_sym(tcc_state, func, &tcc_state->tccgen_func_old_type, 0);
    if (!sym->c)
